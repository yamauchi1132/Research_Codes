...
public:
  ...
  void referEquationOfStateDamping1() {
    ...
    //write now
    double gamma = 1 + ninv;
    this->pres = kconInThisUnit * pow(this->dens, gamma); 
    this->vsnd = sqrt(gamma * kconInThisUnit * pow(this->dens, gamma-1));
    this->umin = 0.;
    this->temp = 0.;
#endif
  }
  ...
  inline void calcPaczynskiWiitaGravity(PS::F64vec & accg3,
					PS::F64 & pot3) {
#if 0 // a. tanikawa comments out 17/11/16
  ...
  */
  ...
  void correctDamping1(PS::F64 dt) {
    ...
    //write now
    double gamma = 1 + ninv;
    this->acc  -= this->vel * 0.0005;
    this->vel   = this->vel2   + 0.5 * this->acc   * dt;
    this->uene  = this->uene2  + 0.5 * this->udot  * dt;
    this->alph  = this->alph2  + 0.5 * this->adot  * dt;
    this->alphu = this->alphu2 + 0.5 * this->adotu * dt;
    this->uene  = (kconInThisUnit / (gamma - 1)) * pow(this->dens, gamma-1);
#endif
  }    
...
//yamauchi adds this
const PS::F64 HelmholtzGas::nidx = 1.5;
const PS::F64 HelmholtzGas::ninv =  1. / HelmholtzGas::nidx;
const PS::F64 HelmholtzGas::kcon = 2.478135e+14;
...
void restartSimulation(char **argv,
  ...
  */
  ...
  //RP::TimestepHexa = 1.0e+4; //time step control about Hexa file. Yamauchu adds
  // *********************
  RP::outputRunParameter(argv);
  return;
}
