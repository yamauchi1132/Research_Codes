...
struct calcDensity {
    ...
    void operator () (const DensityEPI *epi,
	    ...
	    // omgi_i shold be 1, when omgi_i is "nan".
...
struct calcDensity {
    ...
    void operator () (const DensityEPI *epi,
		...
		/*if(buf_hs == RP::KernelSupportRadiusMaximum){	    
		  printf("%lf 2\n", buf_hs);
		  }*/
                density[i].dens = rh_i;
                density[i].np   = nj_i;
		...
		//printf("%lf %lf %lf\n",  PS::F64(RP::NumberOfDimension), m_j, epj[j].mass);
                ghj += q_i * kw1;
                grdh_i -= ghj * hi4_i * m_j;
...
void calcDensityKernel(Tdinfo & dinfo,
	...
	density.calcForceAllMultiWalk(DispatchKernel_Dens,
				      RetrieveKernel_Dens,
				      tag_max,
				      sph,
				      dinfo,
